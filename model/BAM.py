...
from model.module.PSPNet import OneModel as PSPNet
...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        self.layer0p, self.layer1p, self.layer2p, self.layer3p, self.layer4p = PSPNet_.layer0, PSPNet_.layer1, PSPNet_.layer2, PSPNet_.layer3, PSPNet_.layer4
        if args.svf:
    ...
    def freeze_modules(self, model):
        ...
        for param in model.layer4.parameters():
            param.requires_grad = False
        for param in model.layer0p.parameters():
            param.requires_grad = False
        for param in model.layer1p.parameters():
            param.requires_grad = False
        for param in model.layer2p.parameters():
            param.requires_grad = False
        for param in model.layer3p.parameters():
            param.requires_grad = False
        for param in model.layer4p.parameters():
            param.requires_grad = False
    ...
    def forward(self, x, s_x, s_y, y_m, y_b, cat_idx=None):
        ...
        # Base and Meta
        with torch.no_grad():
            query_feat_0p = self.layer0p(x)
            query_feat_1p = self.layer1p(query_feat_0p)
            query_feat_2p = self.layer2p(query_feat_1p)
            query_feat_3p = self.layer3p(query_feat_2p)
            query_feat_4p = self.layer4p(query_feat_3p)
        base_out = self.learner_base(query_feat_4p)
