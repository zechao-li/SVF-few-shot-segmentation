...
def get_model(args):
    ...
    if hasattr(model,'freeze_modules') and args.svf:
        model.svf_modules(model)
    else:
...
# ----------------------  DATASET  ----------------------
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = dataset_bam.SemData(split=args.split, shot=args.shot, data_root=args.data_root, base_data_root=args.base_data_root, data_list=args.train_list, \
    ...
    if args.evaluate:
        if args.resized_val:
            ...
            val_transform_tri = transform_tri.Compose([
                ...
                transform_tri.Normalize(mean=mean, std=std)])
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset_bam.SemData(split=args.split, shot=args.shot, data_root=args.data_root, base_data_root=args.base_data_root, data_list=args.val_list, \
